    ...
    <div class="container px-3 mx-auto flex flex-wrap flex-col md:flex-row items-center">
		...
		<div class="flex flex-col w-full justify-center items-start text-center md:text-left">
			<h1 class="my-4 text-5xl font-bold leading-tight">
                {{ $page->pages->aboutUs->hero->title }}
            </h1>
			<p class="leading-normal text-2xl mb-8">
                {{ $page->pages->aboutUs->hero->sub_title }}
            </p>
		</div>
    </div>
...
<section class="bg-white border-b py-8">
    <div class="container mx-auto m-8 flex flex-wrap">
        <div class="w-full mb-8">
            <p class="font-semibold text-lg tracking-wide">
                {{ $page->pages->aboutUs->intro->title}}
            </p>
            <p class="mt-4 font-thin">
                {{ $page->pages->aboutUs->intro->text}}
            </p>
        </div>
        @foreach($page->pages->aboutUs->team as $person)
